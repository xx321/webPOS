	...
	<div class="center">
		...
		<div id="main">
			<fieldset>
				<div id="r1">
					...
					<form name="supplierForm">
						<p>
							<label>統一編號:</label> <input name="companyCode" value="${intoGoods.supplier.companyCode}" size="20" maxlength="20" readonly="readonly"> 
								<span> 
									<a href="javascript:void(0)" onclick="listSupplierOpen();"><img src="images/firm.png" width="25" height="25"  /></a>
								</span>
						</p>
						<p>
							<label>廠商名稱:</label> <input name="name" value="${intoGoods.supplier.name}" size="20" maxlength="20" readonly="readonly">
						</p>
						<p>
							<label>聯絡人:</label> <input name="contact" value="${intoGoods.supplier.contact}" size="20"
								maxlength="20" readonly="readonly">
						</p>
						<p>
							<label>電話:</label> <input name="phone" value="${intoGoods.supplier.phone}" size="20" maxlength="20" readonly="readonly">
						</p>
						<p>
							<label>傳真:</label> <input name="fax" value="${intoGoods.supplier.fax}" size="20" maxlength="20" readonly="readonly">
						</p>
						<p>
							<label>E-mail:</label> <input name="email" value="${intoGoods.supplier.email}" size="20"
								maxlength="20" readonly="readonly">
						</p>
					</form>
					<p>
					...
					<form name="taxForm">
					<c:choose>
						<c:when test="${intoGoods.tax==0.05}">
				...
				<div id="r2">
					<s:form name="objectForm" action="updateIntoGoods"
						namespace="/admin">
						<input type="hidden" name="id" value="${intoGoods.id}">
						<input type="hidden" name="supplierId" value="${intoGoods.supplier.id}">
						<input type="hidden" name="status" value="${intoGoods.status}">
						<input type="hidden" name ="closed" value="1">
						<input type="hidden" name="tax" value="${intoGoods.tax}">
						...
						<p>
							<label>發票號碼:</label> <input name="invoiceNumber" value="${intoGoods.invoiceNumber}" size="20"
								maxlength="20">
						</p>
						<p>
							<label>付款方式:</label>
							<c:choose>
								<c:when test="${intoGoods.paymentMode==1}">
									<input type="radio" name="paymentMode" value="1" checked="checked"> 匯款方式
									<input type="radio" name="paymentMode" value="0"> 現金支出
			       				</c:when>
			       				<c:otherwise>
						...
						<p>
							<textarea name="description" rows="5" cols="40">${intoGoods.description}</textarea>
						</p>
			...
			<fieldset id="bads">
				...
				<button type="button" onclick="self.location.href='admin/intoGoods.jsp'">
